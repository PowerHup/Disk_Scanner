...
public:
    ...
    bool manageFileInfo(std::string& command);      //管理文件信息
    bool manageDirInfo(std::string& command);       //管理目录信息
    void showDiskInfo();                            //打印扫盘得到的信息，包括所有子目录数量，文件数，目录层数，目录树深度
    bool showDirInfo(const TCHAR* dirPath, bool& same); //打印指定目录的文件信息
    bool showFileInfo(const TCHAR* filePath);       //打印指定文件的基本信息
};